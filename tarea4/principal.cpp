...
void main_eliminarJugadorConNombreTJugadoresLDE(TJugadoresLDE &listaJugadores);
void main_estaJugadorConNombreEnTJugadoresLDE(TJugadoresLDE &listaJugadores);
void main_obtenerJugadorConNombreTJugadoresLDE(TJugadoresLDE &listaJugadores);
...
void main_crearTTablaJugadores(TTablaJugadores &tabla);
void main_insertarJugadorEnTabla(TTablaJugadores &tabla, TJugador &jugador, TFecha &fecha);
void main_eliminarJugadorDeTTablaJugadores(TTablaJugadores &tabla);
void main_perteneceATTablaJugadores(TTablaJugadores tabla);
void main_obtenerJugadorDeTTablaJugadores(TTablaJugadores tabla);
void main_liberarTTablaJugadores(TTablaJugadores &tabla);
void main_imprimirTTablaJugadores(TTablaJugadores tabla);
//Funciones para testear las aplicaciones
void main_listarEnOrden(TTablaJugadores tabla);
//Funciones para testear colaDePrioridadJugador
void main_crearColaPrioridad(TColaDePrioridadJugador &cp);
void main_invertirPrioridad(TColaDePrioridadJugador &cp);
void main_liberarColaPrioridad(TColaDePrioridadJugador &cp);
void main_insertarColaPrioridad(TJugador &jugador, TColaDePrioridadJugador &cp);
void main_estaVaciaColaPrioridad(TColaDePrioridadJugador cp);
void main_imprimirPrioritario(TColaDePrioridadJugador cp);
void main_eliminarPrioritario(TColaDePrioridadJugador &cp);
void main_estaJugadorEnColaPrioridad(TColaDePrioridadJugador cp);
void main_imprimirPrioridadJugador(TColaDePrioridadJugador cp);
...
int main() {
    ...
    TTablaJugadores tabla = NULL;
    TColaDePrioridadJugador cp = NULL;
    ...
    while (seguir) {
        ...
        }else if (cmd_es("eliminarJugadorConNombreTJugadoresLDE", cmd)) {
            main_eliminarJugadorConNombreTJugadoresLDE(listaJugadores);
            
        }else if (cmd_es("estaJugadorConNombreEnTJugadoresLDE", cmd)) {
            main_estaJugadorConNombreEnTJugadoresLDE(listaJugadores);
            
        }else if (cmd_es("obtenerJugadorConNombreTJugadoresLDE", cmd)) {
            main_obtenerJugadorConNombreTJugadoresLDE(listaJugadores);
        ...
        }else if (cmd_es("crearTTablaJugadores", cmd)) {
            main_crearTTablaJugadores(tabla);

        }else if (cmd_es("insertarJugadorEnTabla", cmd)) {
            main_insertarJugadorEnTabla(tabla, jugador, fecha);

        }else if (cmd_es("eliminarJugadorDeTTablaJugadores", cmd)) {
            main_eliminarJugadorDeTTablaJugadores(tabla);

        }else if (cmd_es("perteneceATTablaJugadores", cmd)) {
            main_perteneceATTablaJugadores(tabla);

        }else if (cmd_es("obtenerJugadorDeTTablaJugadores", cmd)) {
            main_obtenerJugadorDeTTablaJugadores(tabla);

        }else if (cmd_es("liberarTTablaJugadores", cmd)) {
            main_liberarTTablaJugadores(tabla);

        } else if (cmd_es("imprimirTTablaJugadores", cmd)) {
            main_imprimirTTablaJugadores(tabla);
        ...
        ///////////////////////////////////////7
            ...
            main_insertarColaPrioridad(jugador, cp);
        } else if (cmd_es("estaVaciaColaPrioridad", cmd)) {
            main_estaVaciaColaPrioridad(cp);
        } else if (cmd_es("imprimirJugadorPrioritario", cmd)) {
            main_imprimirPrioritario(cp);
        } else if (cmd_es("eliminarJugadorPrioritario", cmd)) {
            main_eliminarPrioritario(cp);
        } else if (cmd_es("estaJugadorEnColaPrioridad", cmd)) {
            main_estaJugadorEnColaPrioridad(cp);
        } else if (cmd_es("imprimirPrioridadJugador", cmd)) {
            main_imprimirPrioridadJugador(cp);
...
void main_eliminarJugadorConNombreTJugadoresLDE(TJugadoresLDE &listaJugadores){
    ...
    char nombre[100];
    leerChars(nombre);
    eliminarJugadorConNombreTJugadoresLDE(listaJugadores, nombre);
}
...
void main_estaJugadorConNombreEnTJugadoresLDE(TJugadoresLDE &listaJugadores){
    ...
    char nombre[100];
    leerChars(nombre);
    if (estaJugadorConNombreEnTJugadoresLDE(listaJugadores, nombre)){
        printf("El jugador con nombre %s está en la lista.\n", nombre);
    }else{
        printf("El jugador con nombre %s NO está en la lista.\n", nombre);
    }
...
void main_obtenerJugadorConNombreTJugadoresLDE(TJugadoresLDE &listaJugadores){
    ...
    leerChars(nombre);
    if (estaJugadorConNombreEnTJugadoresLDE(listaJugadores, nombre)){
        TJugador jugador = obtenerJugadorConNombreTJugadoresLDE(listaJugadores, nombre);
        imprimirTJugador(jugador);
    }else{
        printf("El jugador con nombre %s NO está en la lista.\n", nombre);
    }
...
//////////////////////////////////////
/* Funciones main para JugadoresABB */
//////////////////////////////////////
...
void main_crearTTablaJugadores(TTablaJugadores &tabla){
    assert(tabla == NULL);
    nat max = leerNat();
    tabla = crearTTablaJugadores(max);
}

void main_insertarJugadorEnTabla(TTablaJugadores &tabla, TJugador &jugador, TFecha &fecha){
    assert(tabla != NULL);
    insertarJugadorEnTabla(tabla, jugador, fecha);
    jugador = NULL;
    fecha = NULL;
}
...
void main_eliminarJugadorDeTTablaJugadores(TTablaJugadores &tabla){
    ...
    char nombre[100];
    leerChars(nombre);
    eliminarJugadorDeTTablaJugadores(tabla,nombre);
}
...
void main_perteneceATTablaJugadores(TTablaJugadores tabla){
    ...
    char nombre[100];
    leerChars(nombre);
    if (perteneceATTablaJugadores(tabla, nombre))
        printf("El jugador con nombre %s está en la tabla.\n", nombre);
    else
        printf("El jugador con nombre %s NO está en la tabla.\n", nombre);
}
...
void main_obtenerJugadorDeTTablaJugadores(TTablaJugadores tabla){
    ...
    leerChars(nombre);
    if (perteneceATTablaJugadores(tabla, nombre)){
        TJugador jugador = obtenerJugadorDeTTablaJugadores(tabla, nombre);
        imprimirTJugador(jugador);
    }else{
        printf("El jugador con nombre %s NO está en la lista.\n", nombre);
    }
...
void main_liberarTTablaJugadores(TTablaJugadores &tabla){
    assert(tabla != NULL);
    liberarTTablaJugadores(tabla);
}

void main_imprimirTTablaJugadores(TTablaJugadores tabla){
    assert(tabla != NULL);
    imprimirTTablaJugadores(tabla);
}
...
void main_insertarColaPrioridad(TJugador &jugador, TColaDePrioridadJugador &cp){
    assert(cp != NULL && jugador != NULL);
    insertarEnCP(jugador, cp);
    jugador = NULL;
}
...
void main_estaJugadorEnColaPrioridad(TColaDePrioridadJugador cp){
    ...
    nat id = leerNat();
    if (estaEnCP(id, cp))
        printf("El jugador %d está en la cola de prioridad.\n", id);
    else
        printf("El jugador %d no está en la cola de prioridad.\n", id);
}

void main_imprimirPrioridadJugador(TColaDePrioridadJugador cp){
    assert(cp != NULL);
    nat id = leerNat();
    printf("La prioridad del jugador es %d.\n", prioridad(id, cp));
}
...
void main_colaPrioridadTiempo (){
    ...
    TColaDePrioridadJugador cp ;
    clock_t tm = clock();
    for (nat i = 1; i <= iteraciones; i++) {
        cp = crearCP(N);
        TJugador p;
        for (nat i = N; i >= 1; i--) {
            p = crearTJugador(i, 20 + i, "Uruguay", "Juan", NULL);
            insertarEnCP(p, cp);
            prioritario(cp);
        }
        ...
        liberarCP(cp);
        cp = crearCP(N);
        p = crearTJugador(N, 20 + N, "Uruguay", "Juan", NULL);
        insertarEnCP(p, cp);
        ...
        cp = crearCP(N);
        p = crearTJugador(1, 20 + 1, "Uruguay", "Juan", NULL);
        insertarEnCP(p, cp);
        invertirPrioridad(cp);
        for (nat i = 1; i <= N; i++) {
        	p = crearTJugador(1, 20 + 1, "Uruguay", "Juan", NULL);
        ...
        printf ("El jugador prioritario es: \n");
        imprimirTJugador(prioritario(cp));
        for (nat i = 1; i <= N; i++) {
            estaEnCP(1, cp);
            estaEnCP(N / 2, cp);
